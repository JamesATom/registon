// 20250622194035_create_table_poll.ts
...
export async function up(knex: Knex): Promise<void> {
    ...
    await knex.schema.createTable('pollQuestion', table => {
        table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
        table.uuid('pollId').notNullable();
        ...
        table.foreign('pollId').references('id').inTable('poll').onDelete('CASCADE');
        table.index('pollId');
    });

    await knex.schema.createTable('pollParticipant', table => {
        table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
        table.uuid('pollId').notNullable();
...
export async function down(knex: Knex): Promise<void> {
    await knex.schema.dropTableIfExists('pollParticipant');
    await knex.schema.dropTableIfExists('pollQuestion');
