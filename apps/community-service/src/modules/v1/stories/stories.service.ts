...
import {
  ...
  StoryDocument,
  StoryStatus,
} from '../../shared/models/story.schema';
import {
  StoryItem,
  StoryItemDocument,
} from '../../shared/models/story-item.schema';
...
export class StoriesService {
  ...
  async createStory(
    ...
    const newStory = new this.storyModel({
      ...
      branches: createStoryDto.branches.map(
  ...
  async removeStory(id: string): Promise<Story> {
    ...
    const deletedStoryItems = await this.storyItemModel
      .deleteMany({ storyId: id })
      .exec();
  ...
  async createStoryItem(
    createStoryItemDto: CreateStoryItemDto,
  ): Promise<StoryItem[]> {
  ...
  async updateStoryItem(
    id: string,
    updateStoryItemDto: UpdateStoryItemDto,
  ): Promise<StoryItem> {
    ...
    try {
      ...
      const { title, description, image, orderNumber } =
        updateStoryItemDto.storyItem;
  ...
  async removeStoryItem(
    id: string,
  ): Promise<{ deleted: StoryItem; reordered: number }> {
    ...
    try {
      // Delete the item
      const deletedStoryItem = await this.storyItemModel
      ...
      await this.reorderAfterDelete(
        storyItem.storyId.toString(),
        storyItem.orderNumber,
      );
